...
import { PRINT_CONFIG, wrapPreviewHtmlForPrint } from '@/lib/printone-render';
...
function defaultBack(sender: { name: string; address: string; postalCode: string; city: string }, formaat: string): string {
  ...
  return wrapPreviewHtmlForPrint(backContent, formaat);
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const voorkant = wrapPreviewHtmlForPrint(flyerHtml, formaat);
    const achterkant = backHtml
      ? wrapPreviewHtmlForPrint(backHtml, formaat)
